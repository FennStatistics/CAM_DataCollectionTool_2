...
$(function () {
  ...
  document.getElementById("dialogConfirmSave").title =
    store.language.confirmSaving_01_title; // title confirm saving
});
...
function saveCam() {
  ...
  var CAMnodesFewText = CAMnodes.filter((element) => element.text.length < 3);
  ...
  delay(function () {
    ...
    if (store.flags.usingSupabase) {
      ...
      async function postData() {
        ...
        window.location =
          store.env.linkRedirect + "?participantID=" + store.cam.creator;
      }
      postData();
